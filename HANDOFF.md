...
- 노트 미리보기 카드는 `pdfjs-dist`로 해당 노트 PDF의 1페이지를 실제 렌더링한 썸네일을 표시함
  - 첫 페이지 손글씨 데이터는 PDF 캔버스 위에 합성해서 홈 미리보기에 같이 표시
  - 단, PDF 파일 자체 영속 저장이 아직 없어서 PDF가 연결되지 않은 노트는 현재 기본 데모 PDF fallback을 사용
...
- 오늘 결론:
  ...
  - 홈 노트 카드에는 실제 PDF 1페이지 썸네일과 첫 페이지 손글씨 합성 미리보기를 1차 구현 완료
  - 하지만 기본 노트 생성 흐름, PDF 파일 영속 저장, 폴더 중첩 DB 구조, 프론트 구조 정리는 아직 남아 있음
...
- 업로드 이미지/PDF 파일 저장 구조 설계
  - 현재 새 노트는 기본 데모 PDF fallback에 의존하는 상태
  - 앞으로는 `노트 작성`을 누르면 빈 기본 노트가 생성되고, `PDF 불러오기`를 누르면 저장되어 있는 PDF를 선택해서 현재처럼 노트에 연결되는 흐름 필요
- 노트 상세 화면 페이지 추가 기능 구현
  - 상세 화면에서 새 페이지를 추가하고 DB `note_pages`에 저장/복원하는 흐름 필요
- 프론트의 DB 저장 흐름 안정화 및 리팩터링

### 우선순위 중간
- 우측 메인화면 노트 미리보기 polish
  - 현재는 PDF 1페이지 렌더링과 첫 페이지 손글씨 합성까지 1차 구현됨
  - 추후 실제 업로드 PDF 영속 저장이 연결되면 기본 PDF fallback 없이 각 노트의 실제 파일을 미리보기에 사용해야 함
